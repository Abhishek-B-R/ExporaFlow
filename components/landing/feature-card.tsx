...
export const FeatureCard = ({ feature }: { feature: FeatureItem }) => {
  ...
  return (
    <div className="ef-card h-full p-5 flex flex-col">
      <div className="h-9 w-9 rounded-lg bg-(--surface-3) flex items-center justify-center text-(--accent)">
        <Icon className="size-4" strokeWidth={2} />
      </div>
      <h3 className="text-base font-semibold text-(--foreground) mt-4 tracking-tight">
        {feature.heading}
      </h3>
      <p className="text-sm text-(--muted-2) mt-2 leading-relaxed flex-1">
        {feature.description}
      </p>
