...
export const WorkflowToptileLayout: React.FC<WrapperProps> = ({
  ...
  return (
    <div className="border h-10 rounded border-[color:var(--border)] bg-[color:var(--surface-1)] flex items-center justify-between px-4">
      <div className=" flex gap-x-2 items-center ">
        <Link
          href={`/workflow/${tileTitle.toLowerCase()}`}
          className="flex items-center rounded text-[12px] sm:text-[13px] md:text-[15px] border border-transparent hover:border-[color:var(--border-strong)] px-2 h-7 hover:bg-[color:var(--surface-2)] transition-all duration-300"
        >
          {tileTitle}
        </Link>
